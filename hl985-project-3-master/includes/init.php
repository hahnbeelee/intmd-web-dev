...
define('SESSION_COOKIE_DURATION', 60*60*1); //1 hour
...
function log_in($username, $password){
  ...
  if( isset($username) && isset($password) ){
    ...
    if($records) {
      //username is unique so record should be in position 0 in array
      $account = $records[0];
      ...
      if ( password_verify($password, $account['password']) ) {
        ...
        if ($result){
          ...
          array_push($session_messages, "Log in falied.");
        }
      } else{
...
function session_login(){
  ...
  if (isset($_COOKIE['session'])) {
    ...
    if( isset($session_record) ){
      ...
      //renew cookie for one more hour
      setcookie("session", $session, time() + SESSION_COOKIE_DURATION);
...
function log_out(){
  ...
  //remove the session from the cookie and force it to ecpire (go back in time)
...
if ( isset($_POST['login']) && isset($_POST['username']) && isset($_POST['password']) ){
  ...
  //check if alreagy logged in via cookie
  session_login();
}

//check if we should logout user
